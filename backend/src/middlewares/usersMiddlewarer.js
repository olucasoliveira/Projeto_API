...
const validarEdicaoNome = (request, response, next) => {
    ...
    if(body.nome == ''){
        return response.status(400).json( { message: "O nome não pode ser removido"});
    }
...
const validarEmail = (request, response, next) => {
    ...
    if (parts[0].length > 64) {
        return response.status(400).json({ message: "A parte local do Email é muito longa" });
    }

    var domainParts = parts[1].split(".");
    if (domainParts.some(function (part) { return part.length > 63; })) {
        return response.status(400).json({ message: "Uma parte do domínio do Email é muito longa" });
    }
...
const validarEdicaoEmail = (request, response, next) => {
    ...
    if (email === '') {
        return response.status(400).json({ message: "O Email não pode ser removido" });
    }
...
function checkToken(request, response, next) {
    ...
    if (!token) return response.status(401).json({ mensage : "Acesso negado, faça login" });
