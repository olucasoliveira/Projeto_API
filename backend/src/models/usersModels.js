...
const createUser = async (user) => {
    ...
    try {
        ...
        const [createdUser] = await conn.execute(query, [nome, email, senha]);
...
const checkLogin = async (body) => {
    try {
        const { email } = body;
        const query = 'SELECT id, senha, email FROM USUARIO WHERE EMAIL = ? AND ATIVO = 1';
        const [userExistRows] = await conn.execute(query, [email]);
        if (userExistRows.length === 0) {
            return 0; // Usuário não existe
        } else {
...
const checkAdminLogin = async (body) => {
    try {
        const { email } = body;
        const query = 'SELECT id, senha, email FROM USUARIO WHERE EMAIL = ? AND ATIVO = 1 AND ADMIN = 1';
        const [userExistRows] = await conn.execute(query, [email]);
        if (userExistRows.length === 0) {
            return 0; // Usuário não existe
        } else {
...
const updateUser = async (id, user) => {
    ...
    const { nome, email, senha } = user;
    const query = 'UPDATE USUARIO SET NOME = ?, EMAIL = ?, SENHA = ?  WHERE ID = ? AND ATIVO = 1';
