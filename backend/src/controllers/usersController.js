...
const loginAdminUser = async (request, response) => {
    ...
    //Validando se o usuario admin existe.
    const checkSenha = await usersModel.checkAdminLogin(request.body);
    ...
    if(checkSenha == 0 || checkSenha == "0"){
        ...
        if (resultSenha == false){
            ...
            return response.status(200).json({ message: "Autenticação de administrador realizada com sucesso", token: gerarToken});
        }
    }
    return 0;
}
...
const updateUser = async (request, response) => {
    ...
    if( checkEmail != 0 || checkEmail != "0"){
        if(checkEmail[0] != id){
            return response.status(400).json( { message: "Esse email já esta em uso, escolha outro"});
        }
    }
    ...
    if(updateUser == true){
        return response.status(200).json({ message: "Usuario editado com sucesso"});
    }else{
